...
// Verify data
  ...
  if (array_key_exists("submit", $_POST)) {
    ...
    $query = "SELECT id, hash FROM `parents` WHERE email = '" . mysqli_real_escape_string($link, $email) . "' AND hash= '" . mysqli_real_escape_string($link, $hash) . "' LIMIT 1" ;
    $result = mysqli_query($link, $query) ;
    $row = mysqli_fetch_array($result) ;
    if (mysqli_num_rows($result) > 0) {
// We have a match
      $query = "UPDATE `parents` SET `password` ='" . md5(md5($row['id']) . $Password) . "' WHERE email = '" . mysqli_real_escape_string($link, $email) . "' LIMIT 1" ;
