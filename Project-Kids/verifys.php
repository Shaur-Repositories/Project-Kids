...
if(isset($_GET['email']) && !empty($_GET['email']) AND isset($_GET['hash']) && !empty($_GET['hash'])){
    ...
    $query = "SELECT id, hash, active FROM `schools` WHERE email = '" . mysqli_real_escape_string($link, $email) . "' AND hash= '" . mysqli_real_escape_string($link, $hash) . "' AND active = '0' LIMIT 1";
    $result = mysqli_query($link, $query);

    if(mysqli_num_rows($result) > 0){
        // We have a match, activate the account

        $query = "UPDATE `schools` SET active ='1' WHERE email = '" . mysqli_real_escape_string($link, $email) . "' LIMIT 1";
